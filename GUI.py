...
class KTVWorker(QThread):
    ...
    def __init__(self, youtube_url, output_dir="output", bg_image="black.jpg", with_mv=False):
        super().__init__()
        self.youtube_url = youtube_url
        self.output_dir = output_dir
        self.bg_image = bg_image
        self.with_mv = with_mv
    ...
    def run(self):
        try:
            ...
            '''
            cmd = ["python", "ktv_tool.py", "--input", self.youtube_url, "--gpu", "0"]
            if self.with_mv:
                cmd.append("--with_mv")
...
class KTVApp(QWidget):
    ...
    def initUI(self):
        ...
        self.bgMode = False  # False = black background, True = with MV
        self.btnToggleBG = QPushButton("🎥 目前模式：黑底字幕（點我切換）")
        self.btnToggleBG.clicked.connect(self.toggleBGMode)
        ...
        layout.addWidget(self.btnToggleBG) #new!
    ...
    def processAudio(self):
        ...
        self.worker = KTVWorker(self.youtube_url, self.output_dir, self.bg_image, with_mv=self.bgMode)
    ...
    def toggleBGMode(self):
        ...
        if self.bgMode:
            self.btnToggleBG.setText("🎞️ 目前模式：MV背景字幕（點我切換）")
        else:
            self.btnToggleBG.setText("🎥 目前模式：自訂背景字幕（點我切換）")
